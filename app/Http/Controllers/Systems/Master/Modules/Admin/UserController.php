...
use App\Services\Systems\Master\Crud\CrudRoleService;
use App\Services\Systems\Master\Crud\CrudUserService;
use App\Services\Common\Rules\RulesMaintenanceService;
use App\Services\Systems\Master\Crud\CrudParameterService;
use App\Http\Requests\Systems\Master\Modules\admin\SaveUsersRequest;
use App\Http\Requests\Systems\Master\Modules\admin\UpdateUsersRequest;
...
class UserController extends Controller
{
    ...
    public function usersList()
    {
        try {
            ...
            $viewPath = "legacy.systems.master.modules.admin.pages.users.users_list";
    ...
    public function usersFilters(Request $request)
    {
        try {
            ...
            if (Auth::user()->roles_id == EnumMasterRoles::MANAGER) {
                $query = $query->where("users.roles_id", "!=", EnumMasterRoles::ADMIN);
            }
            ...
            return DataTables::of($query)
                ->addColumn("actions", function ($item) {
                    return view(
                        "legacy.systems.master.modules.admin.pages.users._includes.actions_grid",
                        compact("item")
                    )->render();
                })
    ...
    public function getUserHistory($userId)
    {
        try {
            ...
            return response()->json([
                "status" => 1,
                "grid" => view(
                    "legacy.systems.master.modules.admin.pages.users._includes.tbody_audit_modal",
                    compact("userAudit")
                )->render(),
            ]);
        } catch (QueryException $e) {
    ...
    public function usersMaintenance($id = 0)
    {
        try {
            ...
            $viewPath = "legacy.systems.master.modules.admin.pages.users.users_maintenance";
